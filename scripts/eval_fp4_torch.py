...
class Args:
    checkpoint: str = "/home/taco/checkpoints/pi05_libero_onnx_compat"
...
def eval_libero(args: Args) -> None:
    ...
    # --- START FP4 LOADING ---
    print("Initializing NVIDIA ModelOpt for FP4 Simulation...")
    ...
    # Heuristic search for model
    model_attr = None
    if hasattr(policy, "model"): model_attr = "model"
    elif hasattr(policy, "_model"): model_attr = "_model"  # <--- Found it
    elif hasattr(policy, "policy"): 
        print("Found .policy, checking inside...")
        if hasattr(policy.policy, "model"): model_attr = "policy.model"
    
    if model_attr:
        print(f"Found model at policy.{model_attr}")
        target_model = eval(f"policy.{model_attr}")
    else:
        print("❌ Could not find .model attribute! Checking vars...")
        try:
           print(vars(policy).keys())
        except: pass
        sys.exit(1)

    # 1. Apply FP4 Structre
    print("Applying FP4 Quantization Structure...")
    if hasattr(mtq, "NVFP4_DEFAULT_CFG"):
        qconfig = mtq.NVFP4_DEFAULT_CFG
    else:
        # Fallback (must match export script)
        qconfig = QuantizeConfig(qformat="fp4", groupsize=128, quantize_weights=True, quantize_activations=True)
    
        # We need to exclude the same modules as in export
        # The default config might not have the exclusions!
        # Re-creating the exact config from export script
    ...
    # Quantize
    print(f"Quantizing {model_attr}...")
    quantized_model = mtq.quantize(target_model, qconfig)
    
    # Assign back
    if model_attr == "model": policy.model = quantized_model
    elif model_attr == "policy.model": policy.policy.model = quantized_model
    
    # 2. Load Quantized State Dict
    ckpt_path = os.path.join(args.checkpoint, "thor_fp4_ckpt", "quantized_model.safetensors")
    print(f"Loading Quantized Weights from {ckpt_path}...")
    
    if os.path.exists(ckpt_path):
        sd = load_file(ckpt_path)
        # Load strict=False because some metadata might be missing or mismatched keys
        target_model.load_state_dict(sd, strict=False) 
        print("✅ FP4 Weights Loaded Successfully!")
    else:
